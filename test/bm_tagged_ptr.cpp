...
template <typename T> static void unique_ptr_creation(benchmark::State &state) {
  while (state.KeepRunning()) {
    benchmark::DoNotOptimize(std::unique_ptr<T>(new T()));
  }
}

template <typename T> static void tagged_ptr_creation(benchmark::State &state) {
  while (state.KeepRunning()) {
    benchmark::DoNotOptimize(sp::make_tagged<T, 4>());
  }
}

template <typename T> static void unique_ptr_access(benchmark::State &state) {
...
template <typename T> static void tagged_ptr_access(benchmark::State &state) {
